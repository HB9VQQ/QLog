...
void PSTRotDrv::setPosition(double in_azimuth, double in_elevation)
{
   ...
   // PstRotatorAz handles rotor command correction via its BD setting.
   // QLog only applies offset on readback (display) and re-sends
   // position on band change so PstRotatorAz updates its display.
   qCDebug(runtime) << "Sending azimuth" << in_azimuth << "band:" << currentBand;
